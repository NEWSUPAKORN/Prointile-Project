...
public class RPGBuilderEditor : EditorWindow
{
    ...
    private void DrawEntryList(List<DatabaseEntrySlot> entryList)
    {
        ...
        if (filtersNeedValidation)
        {
            ...
            else
            {
                ...
                float progress;
                progress = entryList.Count > 0 ? (float) curElementChecked / (float) maxElementsToCheck : 0;
    ...
    private void DrawModuleFiltersView()
    {
        ...
        for (var index = 0; index < currentFilterList.Count; index++)
        {
            ...
            if (!string.IsNullOrEmpty(filterEntry.fieldName))
            {
                ...
                fieldName = GetFieldDisplayName(filterEntry.moduleName, filterEntry.categoryName,
    ...
    private string GetEntryFileName(RPGBuilderEditorModule module, string fileName)
    {
        foreach (var entryFound in Resources.LoadAll<RPGBuilderDatabaseEntry>(
            EditorSettings.DatabasePath + module.AssetFolderName))
        {
            if (entryFound.entryFileName != fileName) continue;
            return entryFound.name;
        }
    ...
    public string GetTypeEntryFileNameByName(RPGBuilderEditorModule module, string entryName)
    {
        foreach (var entryFound in Resources.LoadAll<RPGBuilderDatabaseEntry>(
            EditorSettings.DatabasePath + module.AssetFolderName))
        {
            if (entryFound.entryName != entryName) continue;
            return entryFound.entryFileName;
        }

        return "";
    }

    public RPGBuilderDatabaseEntry GetEntryFile(RPGBuilderEditorModule module, string entryFileName)
    {
        return (RPGBuilderDatabaseEntry) AssetDatabase.LoadAssetAtPath(
            EditorData.ResourcePath + EditorSettings.DatabasePath +
