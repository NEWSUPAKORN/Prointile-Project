...
namespace BLINK.RPGBuilder.Managers
{
    public class PetPanelDisplayManager : MonoBehaviour
    {
        ...
        {
            if (GameState.playerEntity.GetTarget() == null) return;
            if (FactionManager.Instance.GetCombatNodeAlignment(GameState.playerEntity, GameState.playerEntity.GetTarget()) == CombatData.EntityAlignment.Ally) return;
            foreach (var pet in GameState.playerEntity.GetCurrentPets())
            {
                pet.SetTarget(GameState.playerEntity.GetTarget());
            }
        }
