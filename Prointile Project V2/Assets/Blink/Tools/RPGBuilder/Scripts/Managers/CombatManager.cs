...
namespace BLINK.RPGBuilder.Managers
{
    public class CombatManager : MonoBehaviour
    {
        ...
        public IEnumerator InitEntityState(CombatEntity casterEntity, CombatEntity targetEntity, RPGEffect effect, int effectRank, Sprite icon, float delay)
        {
            ...
            if (effect.effectType == RPGEffect.EFFECT_TYPE.Mount && targetEntity.IsMounted())
            {
                for (var index = 0; index < targetEntity.GetStates().Count; index++)
                {
                    ...
                    targetEntity.EndStateEffect(index);
                }
                yield break;
            }
        ...
        public void ResetNestedEffects(CombatEntity targetInfo, RPGEffect parentEffect, RPGEffect.RPGEffectRankData parentEffectRank)
        {
            List<int> effectsToRemove = new List<int>();

            for (var index = 0; index < parentEffectRank.nestedEffects.Count; index++)
            {
                var nestedEffect = parentEffectRank.nestedEffects[index];
            ...
            {
                targetInfo.EndStateEffect(effectToRemove);
            }
        }
                    ...
                    switch (effectRank.teleportType)
                    {
                        case RPGEffect.TELEPORT_TYPE.gameScene:
                            if (targetEntity == GameState.playerEntity)
                                RPGBuilderEssentials.Instance.TeleportToGameScene(effectRank.gameSceneID,
                                    effectRank.teleportPOS);
                            break;
