...
namespace BLINK.RPGBuilder.Managers
{
    public class LevelingManager : MonoBehaviour
    {
        ...
        public void AddSkillEXP(int skillID, int _amount)
        {
            foreach (var t in Character.Instance.CharacterData.Skills)
                if (t.skillID == skillID)
                {
                    var skillREF = GameDatabase.Instance.GetSkills()[skillID];
                    ...
                    while (totalAmt > 0)
                    {
                        var XPRemaining = t.maxSkillXP -
                                          t.currentSkillXP;
                        var levelTemplateREF = GameDatabase.Instance.GetLevels()[skillREF.levelTemplateID];
                        if (totalAmt > XPRemaining)
                        {
                            t.currentSkillXP = 0;
                            totalAmt -= XPRemaining;
                            ...
                            // EXECUTE POINTS GAIN REQUIREMENTS
                            GameEvents.Instance.OnSkillLevelChanged(skillREF, t.currentSkillLevel);
                            StatCalculator.UpdateSkillLevelUpStats(skillID);
                            SpawnSkillLevelUpGO();
                        }
                        else
