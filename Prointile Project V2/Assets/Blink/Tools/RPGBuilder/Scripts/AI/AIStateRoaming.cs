...
namespace BLINK.RPGBuilder.AI
{
    public class AIStateRoaming : AIStateIdle
    {
        ...
        protected virtual void HandleRoaming()
        {
            ...
            if (!IsRoamingPaused && NearRoamTarget(roamingTemplate.RoamPointTreshold))
            {
        ...
        protected virtual void GetNewRoamingPoint()
        {
            ...
            {
                RoamTargetPosition = GetValidPoint(ThisAIEntity.ThisCombatEntity.GetSpawner().transform.position, ThisAIEntity.ThisCombatEntity.GetSpawner().areaHeight, ThisAIEntity.ThisCombatEntity.GetSpawner().areaHeight,
                    ThisAIEntity.ThisCombatEntity.GetSpawner().groundLayers);
            }
            else
            {
                RoamTargetPosition = GetValidPoint(ThisAIEntity.transform.position, 40, 50,
                    roamingTemplate.roamGroundLayers);
            }
            ThisAIEntity.StartMovement();
        }
        
        protected virtual Vector3 GetValidPoint(Vector3 basePosition, float height, float maxDistance, LayerMask groundLayers)
        {
            ...
            {
                maxAttempt--;
                pos = GetPoint(basePosition, height, maxDistance, groundLayers);
                if (pos != basePosition)
                {
        ...
        private Vector3 GetPoint(Vector3 basePosition, float height, float maxDistance, LayerMask groundLayers)
        {
            ...
            return Physics.Raycast(spawnPos, -ThisAIEntity.transform.up, out var hit, maxDistance, groundLayers) ? hit.point : basePosition;
        }
