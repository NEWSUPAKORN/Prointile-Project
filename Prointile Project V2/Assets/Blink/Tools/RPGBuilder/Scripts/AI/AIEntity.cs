...
namespace BLINK.RPGBuilder.AI
{
    public class AIEntity : MonoBehaviour
    {
        ...
        [HideInInspector] public int currentPhaseIndex;
        ...
        protected static readonly int movementSpeedParameter = Animator.StringToHash("MovementSpeedModifier");
        ...
        public IEnumerator EnterNewPhase(float transitionTime, int phaseIndex)
        {
            ...
            if (phaseIndex > 0)
            {
                ...
                TriggerPhaseActions(ActivationType.Completed);
            }
            else
            {
               InitNPCPreset();
            ...
            TriggerPhaseActions(ActivationType.Start);
        ...
        public virtual void InitNPCPreset()
        {
            ...
            {
                GameObject rendererGO = RPGBuilderUtilities.GetChildByName(gameObject, preset.RendererName);
                if (rendererGO != null) ThisCombatEntity.mainRenderer = rendererGO.GetComponent<Renderer>();
                if (ThisCombatEntity.mainRenderer == null)
                    ThisCombatEntity.mainRenderer = GetComponentInChildren<Renderer>();
            }

            if(ThisCombatEntity.GetNPCData().isNameplateEnabled) UIEvents.Instance.OnUpdateNameplate(ThisCombatEntity);
        ...
        protected virtual void TriggerPhaseActions(ActivationType activationType)
        {
            if (ThisCombatEntity.GetNPCData().Phases[currentPhaseIndex].PhaseTemplate.ActionsTemplate == null) return;
            foreach (var action in ThisCombatEntity.GetNPCData().Phases[currentPhaseIndex].PhaseTemplate.ActionsTemplate
                .PhaseActions)
            {
                if (action.GameActionsTemplate == null) return;
                if (action.RequirementsTemplate == null || RequirementsManager.Instance.RequirementsMet(
                    action.RequirementsTarget == RPGCombatDATA.TARGET_TYPE.Caster
                        ? ThisCombatEntity
                        : ThisCombatEntity.GetTarget(), action.RequirementsTemplate.Requirements).Result)
                {
                    GameActionsManager.Instance.TriggerGameActions(
                        action.ActionsTarget == RPGCombatDATA.TARGET_TYPE.Caster
                            ? ThisCombatEntity
                            : ThisCombatEntity.GetTarget(), action.GameActionsTemplate.GameActions);
                }
            }
        }
        ...
        {
            if (!CanAnimate()) return;
            EntityAnimator.SetFloat(movementSpeedParameter, modifier);
        }
